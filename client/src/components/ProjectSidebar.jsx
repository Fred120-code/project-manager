...
const ProjectSidebar = () => {
  ...
  const [expanedProjects, setExpanedProjects] = useState(new Set());
  ...
  const toggleProject = (id) => {
    const newSet = new Set(expanedProjects);
    newSet.has(id) ? newSet.delete(id) : newSet.add(id);
    setExpanedProjects(newSet);
  };
  return (
    <div className="mt-6 px-3">
      <div className="flex items-center justify-between px-3 py-2">
        ...
        <Link to="/projects">
          <button
            className="size-5 text-gray-500 dark:text-zinc-400 hover:text-gray-900
           dark:hover:text-white hover:bg-gray-100 dark:hover:bg-zinc-800 rounded flex items-center
          ...
          <div key={project.id}>
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 rounded-lg
                 transition-colors duration-200 text-gray-700 dark:text-zinc-300
                 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-gray-900
                  dark:hover:text-white"
            >
              <ChevronRightIcon
                className={`size-3 text-gray-500 dark:text-zinc-400
                 transition-transform duration-200 ${expanedProjects.has(project.id) && "rotate-90"}`}
              />
              <div className="size-2 rounded-full bg-blue-500" />
              <span className="truncate max-w-40 text-sm">{project.name}</span>
            </button>

            {expanedProjects.has(project.id) && (
