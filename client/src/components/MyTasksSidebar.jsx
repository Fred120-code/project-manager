...
const MyTasksSidebar = () => {
  ...
  const toggleTasks = () => setShowMyTasks((prev) => !prev);

  const getTasksStatusColor = (status) => {
    switch (status) {
      case "DONE":
        return "bg-green-500";
      case "IN_PROGRESS":
    ...
    <div className="mt-6 px-3">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span
            className="bg-gray-200 dark:bg-zinc-700 text-gray-700 dark:text-zinc-300
              ...
              myTasks.map((task, index) => (
                ...
                >
                  <div className="flex items-center gap-2 px-3 py-2 w-full min-w-0">
                    <div
                      className={`w-2 h-2 rounded-full ${getTasksStatusColor(task.status)} shrink-0`}
                    />
